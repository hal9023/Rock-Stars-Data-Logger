                    ...
                    if ser.in_waiting > 0:
                        ...
                        if line:
                            try:
                                ...
                                logfile.write(f"{timestamp}, {pressure_value}\n")
                                logfile.flush()
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.H1("Live Pressure Data Logger", 
                style={
                    'textAlign': 'center', 
                    'marginBottom': 20, 
                    'paddingTop': 20,
...
def update_live_graph(n, view_mode):
    ...
    print(f"[DEBUG] update_live_graph called; buffer size={len(current_data)}")
    if current_data:
        print(f"[DEBUG] sample values: {current_data[-5:]}" if len(current_data)>5 else f"[DEBUG] sample values: {current_data}")
    
    # if connection lost and we still have data, force full view
    if not is_recording and current_data:
        view_mode = 'all'
    ...
    if not current_data:
        ...
        return fig, "Duration: 0.0s", "Data Points: 0", "Status: Waiting for data...", 0
    ...
    return fig, duration_text, points_text, status_text, elapsed
...
if __name__ == '__main__':
    print("Starting Live Pressure Data Logger...")
    print("\nData Source: REAL ARDUINO")
